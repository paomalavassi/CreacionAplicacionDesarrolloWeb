...
// Consulta de productos cómodos / activos con existencias
@Query("SELECT p FROM Producto p WHERE p.precio BETWEEN :precioMin AND :precioMax AND p.activo = true AND p.existencias > 0 ORDER BY p.precio ASC")
public List<Producto> consultaPreciosComodos(@Param("precioMin") double precioMin,
